...
/**
 * "I'm interested in" options. The first six come from the approved content
 * deck; "Request a Quote" and "Technical Support" are retained because the
 * product pages deep-link into this form with those subjects pre-selected.
 */
const subjects = [
  ...enquiryInterests.slice(0, -1),
  "Request a Quote",
  "Technical Support",
  "Other",
];
...
export function ContactForm() {
  ...
  const [form, setForm] = useState({
    ...
    email: "",
    phone: "",
    subject: product ? "Request a Quote" : subjects[0],
    ...
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-line bg-base-800 p-10 text-center shadow-card">
        ...
        <p className="mt-2 max-w-sm text-sm text-fog">
          Your enquiry has reached our sales desk and a confirmation is on its way to your inbox.
  ...
  return (
    <form onSubmit={onSubmit} className="rounded-2xl border border-line bg-base-800 p-6 shadow-card sm:p-8">
      <h3 className="font-display text-xl font-bold text-white">Send us an enquiry</h3>
      <p className="mt-1 text-sm text-fog">We respond within one working day.</p>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        ...
        <div className="sm:col-span-1">
          <label className="mb-1.5 block text-sm font-medium text-fog-bright">Organisation</label>
          <input value={form.organisation} onChange={(e) => update("organisation", e.target.value)} className={field} placeholder="Company or department" />
        </div>
        <div>
        ...
        <div className="sm:col-span-2">
          <label className="mb-1.5 block text-sm font-medium text-fog-bright">I&rsquo;m interested in *</label>
